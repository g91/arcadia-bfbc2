...
//#include "show_dump.h"
    ...
    #error "OpenSSL 1.0.2 or older is required because newer versions don't connect to TLS1.0/SSL3"
#endif
...
#define SSL_COMPAT(X)   SSL_CTX_set_mode(X, SSL_MODE_AUTO_RETRY | SSL_MODE_SEND_FALLBACK_SCSV); \
                        SSL_CTX_set_cipher_list(X, SSL_TXT_ALL); \
                        SSL_CTX_set_options(X, SSL_OP_ALL | SSL_OP_LEGACY_SERVER_CONNECT | SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION);
...
void handle_server_info(u8 *data, int datalen, u8 *show_server_info, u32 *ret_ip, u16 *ret_port);
void show_help(void);
...
int main(int argc, char *argv[]) {
    ...
    for(i = 1; i < argc; i++) {
        ...
        switch(argv[i][1]) {
            ...
            case 'X': {
                ...
                show_server_info = argv[i];
                if(!show_server_info) show_server_info = "";
                if(!stricmp(show_server_info, "none")) show_server_info = ""; // compatibility with old example
                } break;
            case 'C': {
    ...
    */
    ...
    //if(!fesl_sd) {
        ...
        fprintf(stderr, "- target   %s (%s) : %hu\n", inet_ntoa(peer.sin_addr), mshost, ntohs(peer.sin_port));
        ...
        if(connect(fesl_sd, (struct sockaddr *)&peer, sizeof(peer))
          < 0) std_err();
        if(dossl) {
            ctx_sd = SSL_CTX_new(SSLv23_method());
            if(!ctx_sd) GOTO_QUIT
            SSL_COMPAT(ctx_sd)
            ssl_sd = SSL_new(ctx_sd);
            if(!ssl_sd) GOTO_QUIT
            #ifdef SSL_set_tlsext_host_name
            SSL_set_tlsext_host_name(ssl_sd, mshost);
            #endif
            if(!SSL_set_fd(ssl_sd, fesl_sd)) GOTO_QUIT;
            j = SSL_connect(ssl_sd);
            if(j <= 0) {
                fprintf(stderr, "SSL_connect error %d %d\n", j, SSL_get_error(ssl_sd, j));
                GOTO_QUIT
            }
        } else {
            ssl_sd = NULL;
...
get_list:
    ...
    if(fesl_sd) {   // try to make it only one time
        ...
        if(send_custom_dest && (tolower(send_custom_dest[0]) == 'f')) { // fesl
            if(send_custom_func(ssl_sd, fesl_sd, 0xc0000003) < 0)GOTO_QUIT
        }
    ...
    if(!sd) {   // reuse the connection if it's still up
        ...
        // long story short, often the server doesn't reply and we need to do it again
        // no idea what's the problem since there is no SSL, just a plain connection
        for(i = 3; i > 0; i--) {
            sd = create_socket();
            if(sd < 0) std_err();
    ...
    if(send_custom_dest && (tolower(send_custom_dest[0]) == 't')) { // theater
        if(send_custom_func(NULL, sd, 0x40000000) < 0)GOTO_QUIT
    }
...
int show_server_info_match(u8 *match, u8 *par) {
    u8      *p,
            *l;
        ...
        if(val) {
            ...
            *val++ = 0;
            ...
            if(par && ((strlen(infostr) + 1 + strlen(par) + 1 + strlen(val) + 1) < sizeof(infostr))) {
                if(!show_server_info_match(show_server_info, par)) {
...
void show_help(void) {
    fprintf(stdout, "\n"
        ...
        "-X [FILTER]    show the informations for each server instead of the IP list,\n"
        "               example of optional FILTER: -X \\hostname\\hostport\\maxplayers\n"
...
int recv_tcp(SSL *ssl_sd, int sd, u8 *data, int datalen) {
    ...
    for(len = 0; len < datalen; len += t) {
        if(!ssl_sd) {   // timeout can't be used with ssl because ssl is a layer over TCP
            if(timeout(sd, 5 /*was 10*/) < 0) return(-1);
        }
...
int fake_fesl_server(int sd) {
    ...
    if(dossl) {
        ...
        if(SSL_accept(ssl_sd) < 0) GOTO_QUIT
    }
        ...
        */
        ...
        if(TXN_CMP("Hello")) {
            ...
            if(ea_send(ssl_sd, sd, type, 0x80000000 | ++cnt,
                ...
                curtime,
                p, l ? "-" : "", l ? (char*)l : "",
                fake_server_port + 5) < 0) GOTO_QUIT
            ...
            if(ea_send(ssl_sd, sd, type, 0x80000000 | ++cnt,
                ...
                "%s.[]=0\n",
                txn,
                (!strnicmp(txn, "Get", 3)) ? txn + 3 : txn) < 0) GOTO_QUIT
        }
    }
